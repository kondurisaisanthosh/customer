...
ui <- dashboardPage(
  ...
  # header
  dashboardHeader(
    title = "Satisfaction Board"
    # tags$li(class = "dropdown",
    #        style = "margin-top: 7px; margin-right: 5px;",
    #       actionButton(icon = icon(""), "help", "", title = "Start a tour of the dashboard"))
  ),
  ...
  dashboardSidebar(
    sidebarMenu(
      ...
      menuItem("Filter", icon = icon("filter"), startExpanded = FALSE,
               radioButtons("center", "Select any", list("Combined", "Support Center A", "Support Center B", "Support Center C", "Support Center D"))
      )
    )
  ),
...
server <- function(input, output, session) {
  ...
  output$map <- renderPlot({
    ...
    mapCountryData(matched, nameColumnToPlot = "value", mapTitle = "", 
                   catMethod = "pretty", colourPalette = c("lightblue", "red"))
  })
}
